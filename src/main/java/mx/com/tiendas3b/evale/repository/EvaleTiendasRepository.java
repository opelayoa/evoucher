...
public interface EvaleTiendasRepository extends CrudRepository<EvaleTiendas, Integer> {
	...
	@Query(value = "call evale_valida_llave(?1, ?2);", nativeQuery = true)
	EvaleTiendas validateKey(String key, String tclave);

	@Query(value = "call evale_trae_llave(?1);", nativeQuery = true)
	EvaleTiendas getKey(String tclave);
	
	@Modifying
	@Transactional
	@Query(value = "call evale_registra_llave(?1, ?2, ?3, ?4);", nativeQuery = true)
